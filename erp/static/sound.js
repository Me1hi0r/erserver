...
function selectRiddle(num) {
    ...
    byId('npTitle').innerText = TYPE + '/' + last_topic;
    ...
    fetch(`sound/${TYPE}/${RIDDLE}`)
        ...
        .then((data) => {
            ...
            for (let s of data['sounds'])
                out += `<li><div class="sound-item" id='${s}' onclick="selectSound('${s}')">${s.slice(8, -4)}</div></li>`;
            out += `</ul>`;
...
function selectSound(track) {
    setRed();
    playSound(`/erp/media/${TYPE}/${track}`);
    NAME = track;
    showDrop();
    showSelect();
    last_track = track.slice(8, -4);
    byId('npTitle').innerText = TYPE + '/' + last_topic + '/' + last_track;
