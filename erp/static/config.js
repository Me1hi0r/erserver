...
function auto_slider(num) {
    ...
    if (value == 0) {
        ...
        block.appendChild(div): block.removeChild(block.children[block.children.length - 1]);
    }
}
...
var options = {
    timeout: 3,
    onSuccess: () => {
        console.log("MQTT: Connected: " + MQTT_ADDR + ":" + MQTT_PORT + ", id: " + clientId);
        topic.forEach((e) => {
            mqtt_subscribe(e);
    ...
    onFailure: (message) => {
        console.log("Error: MQTT: Connection: " + MQTT_ADDR + ":" + MQTT_PORT + ": " + message.errorMessage);
    }
};
...
document.addEventListener("DOMContentLoaded", (e) => {
    ...
    byName('playing_time').addEventListener('input', (e) => {
        let val = e.srcElement.value;
        byId('time-set-label').innerText = `game duration ${(val < 10) ? "0":""}${val}m`;
    ...
    byName('back_vol').addEventListener('input', (e) => {
        let val = e.srcElement.value;
        byId('back-vol-label').innerText = `background sound vol ${(val < 10) ? "0":""}${val}`;
        mqtt_publish(val, "/er/mc2/vol/set");
    });

    byName('main_vol').addEventListener('input', (e) => {
        let val = e.srcElement.value;
        byId('activ-vol-label').innerText = `active sound vol ${(val < 10) ? "0":""}${val}`;
